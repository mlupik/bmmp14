 'use strict';
  ...
  Play2.prototype = {
	...
	setupLevel: function() {
		//Hintergrund
		this.background = this.game.add.sprite(0,0,'hintergrund2');
		this.middleground = this.game.add.tileSprite(0,0,4788,900, 'mittelgrund2');
		this.foreground = this.game.add.tileSprite(0,0, 4788,900,'vordergrund2');
		//Tilemap
		this.map = this.game.add.tilemap('tech_map');
		this.map.addTilesetImage('tileeinss');
		this.map.addTilesetImage('tilezwei');
	...
	pauseMenu: function(){
		if(!(this.gameOver ||  this.won)){
			...
			var menu_bg = this.game.add.sprite(0,0,'menu_pause_tech');
			var continue_button = this.game.add.sprite(240,220, 'button_continue_tech');
	...
	gameOverMenu: function(){
		if(!this.won){
			...
			this.window = this.game.add.group();
			var menu_bg = this.game.add.sprite(0,0,'menu_gameover_tech');
			var again_button = this.game.add.button(180,220, 'button_again_tech', function (){this.game.state.start('play2'); }, this);
			var menu_button = this.game.add.button(310, 220, 'button_menu_tech', function() {this.game.state.start('chooseStar');}, this);
			this.window.add(menu_bg);
